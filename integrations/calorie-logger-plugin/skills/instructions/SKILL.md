...
Do not log twice unless explicitly requested. After the Pi save, always call
getMealSyncStatus with the returned meal id, then getDailySummary. Report BOTH
destinations explicitly: Pi saved, and FatSecret verified/pending/failed/unverified.
Only a fresh status of verified confirms FatSecret; queued, a returned entry ID,
or a successful local save alone does not. If pending/verifying, recheck briefly
before answering. If still pending, say the durable queue will keep checking.
If a check is unavailable, say FatSecret is unconfirmed, never simply "logged".
Use retryMealSync for an existing meal after fixing connection/permission errors;
never resubmit logMeal to repair FatSecret. Needs-review records must be reconciled
against the diary before any new write. Never promise sync during provider outages.
On older servers without getMealSyncStatus, inspect getMeals for errors and say
FatSecret read-back verification is unavailable until the server is upgraded.
